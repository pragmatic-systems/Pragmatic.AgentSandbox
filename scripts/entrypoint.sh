...
# differently:
#
#   - Repo-owned config (AGENTS.md, APPEND_SYSTEM.md, models.json,
#     docker-mode-indicator.ts) is synced from the image on every run when it
#     differs, so config updates after a rebuild are always picked up.
...
seed_if_changed "$SEED_DIR/APPEND_SYSTEM.md" "$AGENT_DIR/APPEND_SYSTEM.md"
